...
public class UserMasterSeeder {
    ...
    /** Called by DatabaseSeeder — not a CommandLineRunner itself */
    public void seed() {
        String nickname           = faker.name().firstName();
        String normalizedNickname = sanitizeForEmail(nickname);
        String emailString        = "master@webmaster.com";
        ...
        if (existingUser.isEmpty()) {
            user = User.createMaster(null, email, "temp", normalizedNickname, 1L);
        ...
        if (existingMaster.isEmpty()) {
            Master master    = Master.create(user.getId(), true, false);
            MasterProfile profile = MasterProfile.create(nickname);
